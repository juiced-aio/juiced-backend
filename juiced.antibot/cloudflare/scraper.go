...
func Init(client http.Client, key string, debug bool) (scraper Scraper) {
	...
	scraper.InitHeaders = CreateInitHeaders("")

	scraper.ChallengeHeaders = CreateChallengeHeaders("", "", "", "", "")

	scraper.SubmitHeaders = CreateSubmitHeaders("", "", "", "")
	...
	if scraper.FingerprintChallenge {
		...
		for {
			...
			if scraper.SolveRetries == scraper.SolveMaxRetries {
				...
				req.RawHeader = CreateInitHeaders(scraper.OriginalRequest.Request.UserAgent())
	...
	for {
		...
		if scraper.InitChallengeRetries == scraper.InitChallengeMaxRetries {
			...
			if scraper.KeyStrUriSafe == "" {
				...
				req.RawHeader = CreateChallengeHeaders(fmt.Sprint(len(payload)), scraper.OriginalRequest.Request.UserAgent(), initURLSplit[len(initURLSplit)-1], "https://"+scraper.Domain, strings.Split(scraper.OriginalRequest.Request.URL.String(), "?")[0])
...
func (scraper *Scraper) SubmitChallenge() (*http.Response, error) {
	...
	for {
		...
		if scraper.SubmitFinalChallengeRetries == scraper.SubmitFinalChallengeMaxRetries {
			...
			req.RawHeader = CreateSubmitHeaders(fmt.Sprint(len(payload)), "https://"+scraper.Domain, scraper.OriginalRequest.Request.UserAgent(), scraper.OriginalRequest.Request.URL.String())
