...
func (cc *ClientConn) roundTrip(req *http.Request) (res *http.Response, gotErrAfterReqBodyWrite bool, err error) {
	...
	/* for _, Header := range req.RawHeader {
		if Header[0] == "accept-encoding" && strings.Contains(Header[1], "gzip") {
			requestedGzip = true
		}
	} */
...
func (rl *clientConnReadLoop) processPushPromise(f *MetaPushPromiseFrame) error {
	...
	pr := &PushedRequest{
		...
		OriginalRequestRawHeader: stream.req.RawHeader,
