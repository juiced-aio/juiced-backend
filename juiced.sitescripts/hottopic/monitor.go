...
func CreateMonitor(input entities.MonitorInput, baseMonitor *entities.BaseMonitor) (entities.Monitor, error) {
	...
	case enums.SKUMonitor:
		hotTopicMonitorInput, err := ValidateMonitorInput(input.Input, input.MonitorType, input.SiteSpecificInfo)
		if err != nil {
			return nil, err
		}
		return &SKUMonitor{
			Input:         input,
			HotTopicInput: hotTopicMonitorInput,
...
func (monitor *SKUMonitor) GetProductInfos() ([]entities.ProductInfo, error) {
	...
	sizesJoined := monitor.HotTopicInput.Size
	if sizesJoined != "" {
	...
	colorsJoined := monitor.HotTopicInput.Color
	if colorsJoined != "" {
