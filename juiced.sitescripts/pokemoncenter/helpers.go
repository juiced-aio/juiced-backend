...
func retrievePaymentToken(keyId string) (string, error) {
	...
	// An error was happening that could be ignored
	/* if err != nil {
		return "", err
	} */
...
func (task *Task) HandleDatadome(body string, taskPercent int) {
	status := task.Task.Task.TaskStatus
	task.PublishEvent(enums.WaitingForCaptchaMonitor, enums.MonitorUpdate, taskPercent)
	quit := make(chan bool)
	defer func() {
		quit <- true
	}()
	...
	cookies := task.Task.Client.Jar.Cookies(BaseURL)
	for _, cookie := range cookies {
		if cookie.Name == "datadome" {
			datadomeInfo.CID = cookie.Value
		}
	...
	err = SetDatadomeCookie(datadomeInfo, task.Task.Proxy, &task.Task.Client, &cancellationToken)
	if err != nil {
		log.Println(err.Error())
		return
	} else {
		task.PublishEvent(status, enums.MonitorUpdate, taskPercent)
		log.Println("Cookie updated.")
		return
	}
}
